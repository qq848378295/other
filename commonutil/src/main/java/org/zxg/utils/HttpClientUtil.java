...
public class HttpClientUtil {
        ...
        try {
            ...
            if(headers!=null){
                for (String k:headers.keySet()) {
                    httpGet.setHeader(k,headers.get(k));
                }
            }
    ...
    public static String doPostJson(String url, String json,Map<String, String> cookies) {
        return doPostJson(url,json,null,cookies);
    }
//    Map<String, String> cookies
    public static String doPostJson(String url, String json,Map<String,String> headers,Map<String, String> cookies) {
        // 创建Httpclient对象
        CloseableHttpClient httpClient = HttpClients.createDefault();
        CloseableHttpResponse response = null;
        String resultString = "";
        try {
            ...
            StringEntity entity = new StringEntity(json, ContentType.APPLICATION_JSON);// 创建请求内容
            httpPost.setEntity(entity);
            // 执行http请求
            response = httpClient.execute(httpPost);
            resultString = EntityUtils.toString(response.getEntity(), "utf-8");
